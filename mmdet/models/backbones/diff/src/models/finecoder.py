...
class DiftStrideDeepFusionFinecoder(nn.Module):
...
class DiftStrideUpsampleFinecoder(nn.Module):
    def __init__(self, in_channels=[768, 384, 192, 96], num_group=24):
        super().__init__()

    def forward(self, x8, x16, x32, x64):

        x4_up = F.interpolate(x8, scale_factor=2,
                              mode='bilinear', align_corners=True)
        x8_up = F.interpolate(x16, scale_factor=2,
                              mode='bilinear', align_corners=True)
        x16_up = F.interpolate(x32, scale_factor=2,
                               mode='bilinear', align_corners=True)
        x32_up = F.interpolate(x64, scale_factor=2,
                               mode='bilinear', align_corners=True)
